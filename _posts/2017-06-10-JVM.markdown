...
좋은 JAVA 개발자는 GC를 고려하여 개발을 해야한다고 한다.(어렵다...)    
...
Old Generation에서 일어나는 GC를 Major GC, Full GC라고 하는데, Major GC를 수행하는 다양한 패턴이 존재한다.  
...
지금까지 JVM에서부터 GC까지 알아보았다.  
개발자로써 계속해서 성장하기 위해서는 자신이 지금 개발하고 있는 언어에 대한 보다 깊은 이해가 필요하다.  JVM의 구조나 동작원리를 이해하고 개발한다면, 보다 효율적인 시스템의 개발이 용이해질 것이라 생각한다.
