...
>> 전체길이 필드가 16비트라서, 단편옵셋도 16비트 만큼 표시가 가능해야 하지만 3비트를 플래그 표시에 사용했으므로, 남은 13비트로 16비트를 표현하기 위해  
  *예) 첫번째 단편의 옵셋 값은 0 : 첫번째 단편이므로 시작 위치가 0임  
  *예) 두번째 단편부터는 이전에 단편화된 길이를 8로 나누어 계산 : 만약, 첫번째가 800바이트였다면 두번째 단편화 옵셋 값은 100    
...
IP에서 현재 대중화된 현재 대중화된 프로토콜은 IPv4이나, 주소공간의 고갈 문제가 있어 IPv6로 전환중에 있다.(IPv4 : 1.160.10.240 | IPv6 : 3ffe:1900:4545:3:200:f8ff:fe21:67cf)    
